...
const getMedicChain = async(req,res) =>{
    try{
        ...
        const query = await client.query(`select id,nombre,correo from cadena_medica where id=$1`,[id]);
...
const deleteMedicChain = async(req,res) =>{
    try{
        const id = req.params.id;
        const client = await getConnection.client;
        await client.query(`delete from cadena_medica where id=$1`,[id]);
...
const updateMedicChain = async(req,res) =>{
    try{
        ...
        if(name !== undefined ){
            await databaseAccess.query(`update cadena_medica set nombre=$1 where id=$2`,[name,id]);
        }

        if(password !== undefined ){
            await databaseAccess.query(`update cadena_medica set contraseña=$1 where id=$2`,[await passwordManager.getEncriptedPassword(password),id]);
        }

        if(email !== undefined ){
            await databaseAccess.query(`update cadena_medica set correo=$1 where id=$2`,[email,id]);
        }
